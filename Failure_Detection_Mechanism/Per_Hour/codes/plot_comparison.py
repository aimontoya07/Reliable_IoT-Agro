...
plt.bar(r3, var_FAR, color ='#d43131', width=barWidht, label='FAR')
plt.bar(r4, var_FS, color ='#099c0e', width=barWidht, label='F-Score')

plt.xlabel('')

#10
# plt.xticks([r + barWidht for r in range(len(var_recall))],['DBSCAN (e=0,2)','IF (c=0,09-0,1)','SVM (nu=0,09-0,1)'])
# 5
# plt.xticks([r + barWidht for r in range(len(var_recall))],['DBSCAN (e=0,3-0,8)','IF (c=0,05-0,08)','SVM (nu=0,09-0,1)'])
#1
plt.xticks([r + barWidht for r in range(len(var_recall))],['DBSCAN (e=0,3-0.8)','IF (c=0-0,04)','SVM (nu=0,01-0,1)'])
